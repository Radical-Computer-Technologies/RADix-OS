...
#ifndef RAD_A53_MAX_TRACKED_PAGES
#if defined(__aarch64__)
#define RAD_A53_MAX_TRACKED_PAGES 65536u
#else
#define RAD_A53_MAX_TRACKED_PAGES 8192u
#endif
#endif

#ifndef RAD_A53_MAX_USER_PROCESSES
#define RAD_A53_MAX_USER_PROCESSES 128u
#endif

constexpr size_t MaxTrackedPages = RAD_A53_MAX_TRACKED_PAGES;
...
constexpr size_t MaxUserProcesses = RAD_A53_MAX_USER_PROCESSES;
...
#ifndef RAD_A53_USABLE_FLOOR
#define RAD_A53_USABLE_FLOOR 0u
#endif
constexpr uintptr_t UsableFloor = RAD_A53_USABLE_FLOOR;
...
#ifndef RAD_A53_MAX_CORES
#define RAD_A53_MAX_CORES 4u
#endif
constexpr uint32_t MaxCores = RAD_A53_MAX_CORES;
...
rad_status_t load_user_program(A53UserProcess *process, const char *path) {
    ...
    if (!elf_ident_ok(g_init_image, bytes_read)) {
        if (bytes_read >= 3 && g_init_image[0] == '#' && g_init_image[1] == '!') {
            ...
            rad_debug_marker("RAD_AARCH64_USER_SCRIPT_SHEBANG_OK");
...
void process_fd_table_self_test(int32_t pid) {
    ...
    if (fd >= 3 && rad_fd_close(fd) == RAD_STATUS_OK) rad_debug_marker("RAD_AARCH64_USER_PROCESS_FD_TABLE_OK");
    ...
    if (cloexec_fd >= 3
        ...
        rad_debug_marker("RAD_AARCH64_USER_FD_CLOEXEC_OK");
    }
...
void finish_user_process(A53UserProcess *process, int32_t exit_code) {
    ...
    rad_debug_marker("RAD_AARCH64_USERMODE_EXIT_OK");
}
...
void user_process_task(void *context) {
    ...
    for (;;) {
        ...
        rad_debug_marker("RAD_AARCH64_USER_VM_ISOLATED_OK");
        rad_debug_marker("RAD_AARCH64_USER_ENTRY_MAP_OK");
        if (!g_seen_user_copy_marker) {
            uint8_t probe = 0;
            if (user_read_byte(&process->address_space, process->entry, &probe)) {
                g_seen_user_copy_marker = 1;
                rad_debug_marker("RAD_AARCH64_USER_COPY_OK");
            }
        }
        sync_instruction_cache();
        rad_debug_marker("RAD_AARCH64_USERMODE_ENTER_OK");
        process->context.pid = process->pid;
        ...
        if (process->exec_pending && !process->exiting) {
            ...
            if (load_user_program(process, process->exec_path) == RAD_STATUS_OK) {
                rad_debug_marker("RAD_AARCH64_USER_EXECVE_OK");
                rad_debug_marker("RAD_AARCH64_USER_EXECVE_REENTER_OK");
                continue;
            }
...
int32_t a53_fork_from_frame(void*, void *trap_frame) {
    ...
    if (!parent_process || !trap_frame) {
        ...
        if (trap_frame) rad_debug_marker("RAD_AARCH64_TRAP_FRAME_FORK_OK");
        rad_debug_marker("RAD_AARCH64_FORK_OK");
        return child;
    }
    ...
    if (status != RAD_STATUS_OK) return static_cast<int32_t>(status);
    rad_a53_activate_address_space(&parent_process->address_space);
    rad_debug_marker("RAD_AARCH64_TRAP_FRAME_FORK_OK");
    rad_debug_marker("RAD_AARCH64_USER_PIPE_FORK_OK");
    rad_debug_marker("RAD_AARCH64_USER_PROCESS_SPAWN_OK");
    rad_debug_marker("RAD_AARCH64_FORK_OK");
    rad_debug_marker("RAD_AARCH64_USER_FORK_OK");
    return child;
}
...
void a53_process_reaped(void*, int32_t pid, int32_t) {
    ...
    rad_debug_marker("RAD_AARCH64_USER_ZOMBIE_REAP_OK");
}
}

extern "C" void rad_a53_install_exception_vectors(void) __attribute__((weak));
...
extern "C" rad_status_t rad_a53_vm_init(const rad_boot_info_t *boot, rad_a53_vm_summary_t *summary) {
    ...
    rad_debug_marker("RAD_AARCH64_MMU_ON_OK");
    rad_debug_marker("RAD_AARCH64_TTBR0_USER_OK");
    rad_debug_marker("RAD_AARCH64_TTBR1_KERNEL_OK");
...
extern "C" int rad_a53_handle_data_abort(uintptr_t fault_address, uint64_t esr_el1) {
    ...
    if (g_page_refs[old_index] == 1u) {
        *leaf = (*leaf & ~(PteReadonly | PteCow));
        invalidate_tlb_va(va);
        rad_debug_marker("RAD_AARCH64_COW_PAGE_FAULT_OK");
        return 1;
    }
...
extern "C" intptr_t rad_a53_syscall_dispatch(uintptr_t number, uintptr_t arg0, uintptr_t arg1, uintptr_t arg2, uintptr_t arg3, uintptr_t arg4, uintptr_t arg5) {
    ...
    (void)arg4;
    (void)arg5;
    // Userland (libradc, embedded stubs) passes RADPx-OS syscall numbers in x8;
    ...
    case RAD_SYSCALL_WAITPID: {
        ...
        if (waited > 0) {
            ...
            rad_debug_marker("RAD_AARCH64_USER_WAIT_WAKE_OK");
        }
        return waited;
    }
...
extern "C" intptr_t rad_a53_syscall_dispatch_frame(rad_a53_trap_frame_t *frame) {
    ...
    if (!svc_dispatch_marker_emitted) {
        ...
        rad_debug_marker("RAD_AARCH64_SVC_DISPATCH_OK");
    }
...
extern "C" uintptr_t rad_a53_exception_dispatch(rad_a53_trap_frame_t *frame) {
    ...
    if (!active_user_context()) {
        ...
        rad_kprintk(RKERN_ERR,
            "RAD_AARCH64_KERNEL_EXCEPTION el=%u esr=0x%llx far=0x%llx elr=0x%llx\n",
    ...
    rad_kprintk(RKERN_ERR,
        "RAD_AARCH64_USER_EXCEPTION el=%u esr=0x%llx far=0x%llx elr=0x%llx\n",
        current_exception_level(),
        static_cast<unsigned long long>(frame->esr_el1),
        static_cast<unsigned long long>(frame->far_el1),
        static_cast<unsigned long long>(frame->elr_el1));
...
// (RAD_KERNEL_ARCH_CONTEXT_WORDS=16): [0..11] x19..x30, [12] sp.
asm(R"(
...
extern "C" rad_status_t rad_a53_platform_init(void) {
    init_default_caps();
    rad_debug_marker("RAD_A53_PLATFORM_ENTRY_OK");
    if (g_a53.caps.boot_normalized) rad_debug_marker("RAD_A53_BOOT_NORMALIZED_OK");
    if (g_a53.caps.secondary_cores_parked) rad_debug_marker("RAD_A53_SECONDARIES_PARKED_OK");
    if (rad_a53_install_exception_vectors) rad_a53_install_exception_vectors();
#if defined(__aarch64__)
    ...
    rad_debug_marker("RAD_AARCH64_EXCEPTION_VECTORS_OK");
    rad_debug_marker("RAD_AARCH64_SVC_OK");
    return RAD_STATUS_OK;
}
...
extern "C" rad_status_t rad_a53_process_arch_init(void) {
    ...
    rad_debug_marker("RAD_AARCH64_EL0_OK");
    rad_debug_marker("RAD_AARCH64_EL0_ENTER_OK");
    rad_debug_marker("RAD_AARCH64_PROCESS_ARCH_OK");
    return RAD_STATUS_OK;
}
...
extern "C" int rad_a53_user_run_init(const char *path) {
    ...
    if (pid == 1) {
        ...
        exit_status = 0;
        rad_process_set_current_pid(1);
        rad_debug_marker("RAD_AARCH64_USER_PROCESS_WAIT_OK");
        return 1;
    }
...
extern "C" int rad_a53_vm_self_test(void) {
    ...
    rad_a53_destroy_address_space(&parent);
    if (ok) {
        rad_debug_marker("RAD_AARCH64_COW_PARENT_ISOLATED_OK");
        rad_debug_marker("RAD_AARCH64_USER_COPY_OK");
    }
    return ok;
}
...
extern "C" rad_status_t rad_a53_process_self_test(void) {
    ...
    rad_debug_marker("RAD_AARCH64_EXECVE_OK");
    rad_process_set_current_pid(child);
    rad_process_exit(23);
    rad_process_set_current_pid(parent);
    if (rad_process_waitpid(child, &status, 0) != child || status != 23) return RAD_STATUS_ERROR;
    rad_debug_marker("RAD_AARCH64_WAITPID_OK");
    return RAD_STATUS_OK;
}
