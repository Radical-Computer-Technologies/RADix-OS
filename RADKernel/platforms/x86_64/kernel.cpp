...
#include "x86_vm.h"
#include "rkconfig.h"
#if defined(RAD_X86_UI_PROFILE_WM)
...
#if defined(RAD_X86_TERMINAL_AUTOTEST_NANO)
...
void mirror_rad_marker_lines_to_serial(const char *text, size_t size) {
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        if (ch == '\r' || ch == '\n') {
            line[pos] = '\0';
            if (pos >= 6 && strncmp(line, "RAD_", 6) == 0) {
...
void append_bytes(const char *text, size_t size) {
    if (!text || !size) return;
    mirror_rad_marker_lines_to_serial(text, size);
...
uint32_t terminal_scale_for(const rad_framebuffer_info_t& info) {
    const char *scale_text = RAD_RKCONFIG_TERMINAL_SCALE;
...
void render_terminal(rad_framebuffer_t framebuffer) {
    ...
    if (!scale_marker_sent && scale >= 2u) {
        rad_debug_marker("RAD_TERMINAL_SCALE_OK");
        scale_marker_sent = 1;
    }
    if (!info.pixels || info.pixel_format != RAD_PIXEL_FORMAT_XRGB8888) {
        rad_framebuffer_clear(framebuffer, g_terminal_theme.background);
        rad_framebuffer_draw_text(framebuffer, 1, 1, "RADPx-OS x86_64 Base Terminal", g_terminal_theme.foreground, g_terminal_theme.background);
...
[[maybe_unused]] void render_terminal_legacy(rad_framebuffer_t framebuffer) {
    ...
    if (!info.pixels || info.pixel_format != RAD_PIXEL_FORMAT_XRGB8888) {
        ...
        rad_framebuffer_draw_text(framebuffer, 1, 1, "RADPx-OS x86_64 Base Terminal", 0x00e5f3ffu, 0x0010273au);
        ...
        terminal_draw_text(info, 2, 1, "RADPx-OS x86_64 Base Terminal", 0x00e5f3ffu, 0x0010273au, scale);
...
void posix_abi_self_test(void) {
    ...
    const char message[] = "RADPx-OS POSIX ABI stdio probe\n";
    ...
    if (pid == 0 && time_status == RAD_STATUS_OK && wrote == static_cast<intptr_t>(sizeof(message) - 1)) {
        append_text("RAD_POSIX_ABI_OK\n");
        rad_debug_marker("RAD_POSIX_ABI_OK");
    } else {
        append_text("RAD_POSIX_ABI_FAIL\n");
        rad_debug_marker("RAD_POSIX_ABI_FAIL");
    }
}
...
void module_lifecycle_self_test(void) {
    ...
    if (registered == RAD_STATUS_OK && unregistered == RAD_STATUS_OK && listed
        ...
        append_text("RAD_MODULE_LIFECYCLE_OK\n");
        rad_debug_marker("RAD_MODULE_LIFECYCLE_OK");
    } else {
        append_text("RAD_MODULE_LIFECYCLE_FAIL\n");
        rad_debug_marker("RAD_MODULE_LIFECYCLE_FAIL");
    }
}
...
void runtime_self_test(void) {
    ...
    if (submit == RAD_STATUS_OK && poll == RAD_STATUS_OK && ran > 0 && g_deferred_work_ran == 1) {
        append_text("RAD_DEFERRED_WORK_OK\n");
        rad_printk("RAD_DEFERRED_WORK_OK\n");
    } else {
        append_text("RAD_DEFERRED_WORK_FAIL\n");
        rad_printk("RAD_DEFERRED_WORK_FAIL\n");
    }
    ...
    if (created == RAD_STATUS_OK && wake == RAD_STATUS_OK && wait == RAD_STATUS_OK) {
        append_text("RAD_WAIT_QUEUE_OK\n");
        rad_printk("RAD_WAIT_QUEUE_OK\n");
    } else {
        append_text("RAD_WAIT_QUEUE_FAIL\n");
        rad_printk("RAD_WAIT_QUEUE_FAIL\n");
    }
    ...
    if (rad_scheduler_info_get(&scheduler) == RAD_STATUS_OK
        ...
        && scheduler.arch[0] != '\0') {
        append_text("RAD_SCHED_CORE_OK\n");
        rad_printk("RAD_SCHED_CORE_OK\n");
    } else {
        append_text("RAD_SCHED_CORE_FAIL\n");
        rad_printk("RAD_SCHED_CORE_FAIL\n");
    }

    if (scheduler.detected_cores > 1 && (scheduler.worker_running_mask & 0x2u)) {
        append_text("RAD_AP_WORKERS_ONLINE_OK\n");
        rad_printk("RAD_AP_WORKERS_ONLINE_OK\n");
    }
    if (scheduler.preemption_enabled) {
        append_text("RAD_PREEMPT_SCHED_OK\n");
        rad_printk("RAD_PREEMPT_SCHED_OK\n");
    }
}
...
void ap_scheduler_stress_self_test(void) {
    ...
    if (created == RAD_STATUS_OK && task && rad_task_join(task) == RAD_STATUS_OK
        ...
        append_text("RAD_AP_SCHED_STRESS_OK\n");
        rad_printk("RAD_AP_SCHED_STRESS_OK\n");
    }
}
...
rad_status_t storage_root_service_start(void *context, const rad_service_config_t*) {
    ...
    append_text("RAD_VIRTIO_BLK_READ_OK\n");
    if (x86_ext4_mount_root("/dev/vda") != RAD_STATUS_OK) return RAD_STATUS_ERROR;
    append_text("RAD_EXT4_MOUNT_OK\n");
    rad_debug_marker("RAD_EXT4_MOUNT_OK");
    if (!x86_ext4_self_test()) return RAD_STATUS_ERROR;
    append_text("RAD_EXT4_ROOTFS_OK\n");
    append_text("RAD_EXT4_RW_OK\n");
    append_text("RAD_ROOTFS_INIT_FOUND\n");
    rad_debug_marker("RAD_EXT4_ROOTFS_OK");
    rad_debug_marker("RAD_EXT4_RW_OK");
    rad_debug_marker("RAD_ROOTFS_INIT_FOUND");
    boot->rootfs_ready = 1;
    append_text("RAD_SERVICE_ROOTFS_OK\n");
    rad_debug_marker("RAD_SERVICE_ROOTFS_OK");
    return RAD_STATUS_OK;
}
...
rad_status_t userspace_init_service_start(void *context, const rad_service_config_t*) {
    ...
    append_text("RAD_USER_INIT_OK\n");
    append_text("RAD_SERVICE_USERSPACE_OK\n");
    rad_debug_marker("RAD_USER_INIT_OK");
    rad_debug_marker("RAD_SERVICE_USERSPACE_OK");
    return RAD_STATUS_OK;
}
...
rad_status_t fatfs_service_start(void *context, const rad_service_config_t*) {
    ...
    if (x86_fat_mount("/dev/vdb", "/mnt/fat") != RAD_STATUS_OK) {
        rad_debug_marker("RAD_FAT_MOUNT_FAIL");
        return RAD_STATUS_ERROR;
    }
    append_text("RAD_FAT_MOUNT_OK\n");
    rad_debug_marker("RAD_FAT_MOUNT_OK");
    if (!x86_fat_self_test()) return RAD_STATUS_ERROR;
    boot->fat_ready = 1;
    append_text("RAD_FAT_RW_OK\n");
    append_text("RAD_SERVICE_FAT_OK\n");
    rad_debug_marker("RAD_FAT_RW_OK");
    rad_debug_marker("RAD_SERVICE_FAT_OK");
    return RAD_STATUS_OK;
}
...
rad_status_t network_smoke_service_start(void *context, const rad_service_config_t*) {
    ...
    append_text("RAD_ETH_TX_OK\n");
    append_text("RAD_ARP_OK\n");
    append_text("RAD_IPV4_OK\n");
    append_text("RAD_UDP_OK\n");
    append_text("RAD_NET_UDP_TX_OK\n");
    append_text("RAD_NET_UDP_RX_OK\n");
    append_text("RAD_NET_HOST_UDP_ECHO_OK\n");
    append_text("RAD_NTP_QUERY_OK\n");
    append_text("RAD_NTP_RESPONSE_OK\n");
    append_text("RAD_NTP_TIME_SAMPLE_OK\n");
    append_text("RAD_SERVICE_NETWORK_OK\n");
    rad_debug_marker("RAD_SERVICE_NETWORK_OK");
    return RAD_STATUS_OK;
}
...
rad_status_t base_terminal_service_start(void *context, const rad_service_config_t*) {
    ...
    append_text("RAD_SERVICE_TERMINAL_OK\n");
    rad_debug_marker("RAD_SERVICE_TERMINAL_OK");
    return RAD_STATUS_OK;
}
...
rad_status_t terminal_stress_service_start(void *context, const rad_service_config_t*) {
    ...
    append_text("RAD_SERVICE_TERMINAL_STRESS_OK\n");
    rad_debug_marker("RAD_SERVICE_TERMINAL_STRESS_OK");
    return RAD_STATUS_OK;
#else
...
rad_status_t register_boot_services(BootServiceContext *context) {
    const rad_service_descriptor_t descriptors[] = {
        {sizeof(rad_service_descriptor_t), "storage-root", "rad,storage-root", "mount-root", 10, storage_root_service_start, nullptr, nullptr, context},
        {sizeof(rad_service_descriptor_t), "userspace-init", "rad,userspace-init", "process-root", 45, userspace_init_service_start, nullptr, nullptr, context},
        {sizeof(rad_service_descriptor_t), "fatfs", "rad,fatfs", "mount-extra", 30, fatfs_service_start, nullptr, nullptr, context},
        {sizeof(rad_service_descriptor_t), "network-smoke", "rad,network-smoke", "network-smoke", 90, network_smoke_service_start, nullptr, nullptr, context},
        {sizeof(rad_service_descriptor_t), "base-terminal", "rad,base-terminal", "terminal", 50, base_terminal_service_start, nullptr, nullptr, context},
        {sizeof(rad_service_descriptor_t), "terminal-stress", "rad,terminal-stress", "terminal-stress", 60, terminal_stress_service_start, nullptr, nullptr, context},
    };
    ...
    append_text("RAD_SERVICE_JSON_OK\n");
    append_text("RAD_SERVICE_BOOTSTRAP_OK\n");
    rad_debug_marker("RAD_SERVICE_JSON_OK");
    rad_debug_marker("RAD_SERVICE_BOOTSTRAP_OK");
    return RAD_STATUS_OK;
}
    ...
    for (size_t i = 0; i < g_boot_service_count; ++i) {
        ...
        if (status != RAD_STATUS_OK) {
            if (strncmp(g_boot_service_entries[i].name, "network-smoke", RAD_SERVICE_NAME_MAX) == 0) {
                append_text("RAD_SERVICE_NETWORK_DEFERRED\n");
                rad_debug_marker("RAD_SERVICE_NETWORK_DEFERRED");
                continue;
            }
            final_status = status;
        }
...
bool nano_autotest_file_matches(void) {
    ...
    rad_vfs_close(file);
    return read == RAD_STATUS_OK && bytes >= 28u && memcmp(buffer, "RADPx-OS nano interactive smoke", 28u) == 0;
}

void nano_autotest_fail(const char *marker) {
    g_nano_autotest.active = false;
    nano_autotest_dump_transcript_tail(marker);
    nano_autotest_log("RAD_STRESS_AUTOTEST_FAIL");
}
...
void nano_autotest_begin(rad_pty_t pty) {
    ...
    nano_autotest_log("RAD_NANO_AUTOTEST_SPAWN_OK");
}
...
void nano_autotest_dump_transcript_tail(const char *marker) {
    ...
    x86_serial_write("RAD_NANO_TRANSCRIPT_TAIL_BEGIN\n");
...
bool nano_autotest_wait_for_nano(uint32_t max_frames) {
    ...
    const uint64_t timeout_ms = static_cast<uint64_t>(max_frames) * 16u;
    if (now - g_nano_autotest.wait_start_ms >= timeout_ms || g_nano_autotest.wait_frames >= timeout_ms) {
        nano_autotest_fail("RAD_NANO_SCREEN_TIMEOUT");
    }
    return false;
}
...
void nano_autotest_poll(void) {
    ...
    if (!g_nano_autotest.poll_seen) {
        ...
        nano_autotest_log("RAD_NANO_AUTOTEST_POLL_OK");
    }
    switch (g_nano_autotest.step++) {
    case 0:
        if (!nano_autotest_wait_for_text("login:", 1200u, "RAD_NANO_LOGIN_PROMPT_TIMEOUT")) {
            --g_nano_autotest.step;
            return;
        }
        nano_autotest_write(g_nano_autotest.pty, "root\n", 5);
        nano_autotest_log("RAD_NANO_AUTOTEST_LOGIN_USER_OK");
        break;
    case 1:
        if (!nano_autotest_wait_for_text("Password:", 1200u, "RAD_NANO_PASSWORD_PROMPT_TIMEOUT")) {
            --g_nano_autotest.step;
            return;
        }
        nano_autotest_write(g_nano_autotest.pty, "rad\n", 6);
        nano_autotest_log("RAD_NANO_AUTOTEST_LOGIN_PASSWORD_OK");
        break;
    case 2:
        ...
        nano_autotest_log("RAD_RASH_PROMPT_SETTLED_OK");
        nano_autotest_write(g_nano_autotest.pty, "sleep-stress\n", strlen("sleep-stress\n"));
        nano_autotest_log("RAD_SLEEP_STRESS_COMMAND_OK");
        break;
    case 3:
        if (!nano_autotest_wait_for_text("sleep-stress-ok", 1200u, "RAD_SLEEP_STRESS_TIMEOUT")) {
            --g_nano_autotest.step;
            return;
        }
        nano_autotest_log("RAD_SLEEP_STRESS_BOOT_OK");
        nano_autotest_write(g_nano_autotest.pty, "signal-stress\n", strlen("signal-stress\n"));
        nano_autotest_log("RAD_SIGNAL_STRESS_COMMAND_OK");
        break;
    case 4:
        if (!nano_autotest_wait_for_text("signal-stress-ok", 1200u, "RAD_SIGNAL_STRESS_TIMEOUT")) {
            --g_nano_autotest.step;
            return;
        }
        nano_autotest_log("RAD_SIGNAL_STRESS_BOOT_OK");
        nano_autotest_write(g_nano_autotest.pty, "posix-stress\n", strlen("posix-stress\n"));
        nano_autotest_log("RAD_POSIX_STRESS_COMMAND_OK");
        break;
    case 5:
        if (!nano_autotest_wait_for_text("posix-stress-ok", 1200u, "RAD_POSIX_STRESS_TIMEOUT")) {
            --g_nano_autotest.step;
            return;
        }
        nano_autotest_log("RAD_POSIX_STRESS_BOOT_OK");
        nano_autotest_write(g_nano_autotest.pty, "tty-stress\n", strlen("tty-stress\n"));
        nano_autotest_log("RAD_TTY_STRESS_COMMAND_OK");
        break;
    case 6:
        if (!nano_autotest_wait_for_text("tty-stress-ok", 1200u, "RAD_TTY_STRESS_TIMEOUT")) {
            --g_nano_autotest.step;
            return;
        }
        nano_autotest_log("RAD_TTY_STRESS_BOOT_OK");
        nano_autotest_write(g_nano_autotest.pty, "tui-stress\n", strlen("tui-stress\n"));
        nano_autotest_log("RAD_TUI_STRESS_COMMAND_OK");
        break;
    case 7:
        if (!nano_autotest_wait_for_text("tui-stress-ok", 2400u, "RAD_TUI_STRESS_TIMEOUT")) {
            --g_nano_autotest.step;
            return;
        }
        nano_autotest_log("RAD_TUI_STRESS_BOOT_OK");
        break;
    default:
        g_nano_autotest.active = false;
        nano_autotest_log("RAD_STRESS_AUTOTEST_OK");
        break;
    }
}
#endif
...
constexpr TerminalStressProgram TerminalStressPrograms[] = {
    {"/usr/bin/sleep-stress", "RAD_SLEEP_STRESS_BOOT_OK", "RAD_SLEEP_STRESS_BOOT_FAIL"},
    {"/usr/bin/signal-stress", "RAD_SIGNAL_STRESS_BOOT_OK", "RAD_SIGNAL_STRESS_BOOT_FAIL"},
    {"/usr/bin/posix-stress", "RAD_POSIX_STRESS_BOOT_OK", "RAD_POSIX_STRESS_BOOT_FAIL"},
    {"/usr/bin/tty-stress", "RAD_TTY_STRESS_BOOT_OK", "RAD_TTY_STRESS_BOOT_FAIL"},
    {"/usr/bin/tui-stress", "RAD_TUI_STRESS_BOOT_OK", "RAD_TUI_STRESS_BOOT_FAIL"},
};
...
void terminal_stress_service_arm(const char *slave_name) {
    ...
    nano_autotest_log("RAD_STRESS_SERVICE_ARMED_OK");
}
...
void terminal_stress_service_poll(void) {
    ...
    if (g_terminal_stress_service.index >= sizeof(TerminalStressPrograms) / sizeof(TerminalStressPrograms[0])) {
        ...
        nano_autotest_log(g_terminal_stress_service.failed ? "RAD_STRESS_AUTOTEST_FAIL" : "RAD_STRESS_AUTOTEST_OK");
        return;
    }
...
void pump_pointer_input(rad_device_t input) {
    ...
    while (rad_input_read_event(input, &event) == RAD_STATUS_OK) {
        if ((event.type == RAD_INPUT_EVENT_POINTER_MOTION || event.type == RAD_INPUT_EVENT_POINTER_BUTTON)
            ...
            rad_debug_marker("RAD_INPUT_POINTER_EVENT_OK");
        }
    }
}
}
...
extern "C" void kernel_main64(uint32_t magic, uint32_t info_addr) {
    ...
    if (topology.detected_cores > 1) {
        x86_serial_write("RAD_SMP_TOPOLOGY_OK\n");
    } else {
        x86_serial_write("RAD_SMP_TOPOLOGY_SINGLE\n");
    }
    ...
    if (x86_cpp_runtime_self_test()) {
        append_text("RAD_CPP_RUNTIME_OK\n");
        rad_debug_marker("RAD_CPP_RUNTIME_OK");
    } else {
        append_text("RAD_CPP_RUNTIME_FAIL\n");
        rad_debug_marker("RAD_CPP_RUNTIME_FAIL");
    }
    ...
    if (register_boot_services(&service_context) != RAD_STATUS_OK) {
        append_text("RAD_SERVICE_BOOTSTRAP_FAIL\n");
        rad_debug_marker("RAD_SERVICE_BOOTSTRAP_FAIL");
    }
...
#endif
    posix_abi_self_test();
    if (x86_cpu_self_test()) append_text("RAD_INT80_OK\n");
    if (x86_context_self_test()) {
        append_text("RAD_CONTEXT_SWITCH_OK\n");
        rad_printk("RAD_CONTEXT_SWITCH_OK\n");
    } else {
        append_text("RAD_CONTEXT_SWITCH_FAIL\n");
        rad_printk("RAD_CONTEXT_SWITCH_FAIL\n");
    }
    if (x86_vm_self_test()) append_text("RAD_VM_PAGE_ALLOC_OK\n");
    if (start_autostart_boot_services() != RAD_STATUS_OK) {
        append_text("RAD_SERVICE_AUTOSTART_FAIL\n");
        rad_debug_marker("RAD_SERVICE_AUTOSTART_FAIL");
    }
    append_text(vm_summary.user_address_space_ready ? "RAD_USER_VM_SCAFFOLD_OK\n" : "RAD_USER_VM_SCAFFOLD_FAIL\n");
    append_text(storage_summary.virtio_block_devices ? "RAD_VIRTIO_BLK_FOUND\n" : "RAD_VIRTIO_BLK_ABSENT\n");
    ...
    snprintf(status_line, sizeof(status_line), "framebuffer pre-slint status=%d pixels=0x%llx\n",
        ...
        static_cast<unsigned long long>(reinterpret_cast<uintptr_t>(framebuffer_probe_info.pixels)));
    x86_serial_write(status_line);
    append_text("RAD_BASE_TERMINAL_OK\n");
    rad_debug_marker("RAD_BASE_TERMINAL_OK");
    ...
    x86_serial_write("cmd test -e /usr/share/terminfo/r/rad\n");
    pty_command(pty, slave, "test -e /usr/share/terminfo/r/rad");
    x86_serial_write("cmd echo rad | wc\n");
    pty_command(pty, slave, "echo rad | wc");
...
#if defined(RAD_X86_DIRECT_LOGIN)
    {
        int32_t login_pid = 0;
        ...
        if (login_status == RAD_STATUS_OK) {
            rad_debug_marker("RAD_DIRECT_LOGIN_SPAWN_OK");
        } else {
            append_text("RAD_DIRECT_LOGIN_SPAWN_FAIL\n");
            rad_debug_marker("RAD_DIRECT_LOGIN_SPAWN_FAIL");
        }
    }
#else
    if (rad_service_start("userspace-init") != RAD_STATUS_OK) {
        append_text("RAD_RADINIT_SPAWN_FAIL\n");
        rad_debug_marker("RAD_RADINIT_SPAWN_FAIL");
    }
#endif
...
#if defined(RAD_X86_UI_PROFILE_WM)
    const rad_status_t slint_status = rad_slint_shell_start(framebuffer, g_transcript);
    if (slint_status == RAD_STATUS_OK) {
        append_text("RAD_X86_UI_PROFILE_WM_OK\n");
        rad_debug_marker("RAD_X86_UI_PROFILE_WM_OK");
        x86_serial_write("RAD_X86_UI_PROFILE_WM_OK\n");
    } else {
        append_text("RAD_X86_UI_PROFILE_WM_FAIL\n");
        rad_debug_marker("RAD_X86_UI_PROFILE_WM_FAIL");
        x86_serial_write("RAD_X86_UI_PROFILE_WM_FAIL\n");
    }
#else
    rad_debug_marker("RAD_X86_UI_PROFILE_TERMINAL_OK");
    x86_serial_write("RAD_X86_UI_PROFILE_TERMINAL_OK\n");
#endif
...
#if defined(RAD_X86_UI_PROFILE_WM)
        if (rad_slint_shell_ready()) rad_slint_shell_set_terminal_text(g_transcript);
    ...
    for (;;) {
        ...
        if (!loop_begin_seen) { loop_begin_seen = true; nano_loop_marker("RAD_NANO_LOOP_BEGIN"); }
#endif
        if (serial) pump_serial_to_pty(serial, pty, slave);
#if defined(RAD_X86_TERMINAL_AUTOTEST_NANO)
        static bool after_serial_seen = false;
        if (!after_serial_seen) { after_serial_seen = true; nano_loop_marker("RAD_NANO_AFTER_SERIAL"); }
        nano_autotest_poll();
        static bool after_autotest_seen = false;
        if (!after_autotest_seen) { after_autotest_seen = true; nano_loop_marker("RAD_NANO_AFTER_AUTOTEST"); }
#endif
#if defined(RAD_X86_UI_PROFILE_WM)
        if (rad_slint_shell_ready()) {
            rad_slint_shell_poll();
        } else if (framebuffer) {
        ...
        if (!after_input_seen) { after_input_seen = true; nano_loop_marker("RAD_NANO_AFTER_INPUT"); }
#endif
        rad_kernel_poll();
#if defined(RAD_X86_TERMINAL_AUTOTEST_NANO)
        static bool after_poll_seen = false;
        if (!after_poll_seen) { after_poll_seen = true; nano_loop_marker("RAD_NANO_AFTER_KERNEL_POLL"); }
#endif
        pty_drain(pty);
#if defined(RAD_X86_TERMINAL_AUTOTEST_NANO)
        static bool after_drain_seen = false;
        if (!after_drain_seen) { after_drain_seen = true; nano_loop_marker("RAD_NANO_AFTER_PTY_DRAIN"); }
#endif
#if defined(RAD_X86_UI_PROFILE_WM)
        if (rad_slint_shell_ready()) {
            if (g_terminal_dirty) rad_slint_shell_set_terminal_text(g_transcript);
        } else if (framebuffer) {
        ...
        if (!after_render_seen) { after_render_seen = true; nano_loop_marker("RAD_NANO_AFTER_RENDER"); }
#endif
        if (x86_ui_idle_frame) x86_ui_idle_frame();
        else rad_task_sleep_ms(16);
#if defined(RAD_X86_TERMINAL_AUTOTEST_NANO)
        static bool after_idle_seen = false;
        if (!after_idle_seen) { after_idle_seen = true; nano_loop_marker("RAD_NANO_AFTER_IDLE"); }
#endif
    }
}
