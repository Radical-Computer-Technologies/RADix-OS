...
#ifndef RAD_KERNEL_MAX_TREE_NODES
#define RAD_KERNEL_MAX_TREE_NODES 64u
#endif

#ifndef RAD_KERNEL_MAX_TREE_PROPS
#define RAD_KERNEL_MAX_TREE_PROPS 192u
#endif

#ifndef RAD_KERNEL_MAX_OVERLAYS
#define RAD_KERNEL_MAX_OVERLAYS 16u
#endif

#ifndef RAD_KERNEL_MAX_I2C_CONTROLLERS
#define RAD_KERNEL_MAX_I2C_CONTROLLERS 8u
#endif

#ifndef RAD_KERNEL_MAX_SPI_CONTROLLERS
#define RAD_KERNEL_MAX_SPI_CONTROLLERS 8u
#endif

#ifndef RAD_KERNEL_MAX_I2C_DEVICES
#define RAD_KERNEL_MAX_I2C_DEVICES 32u
#endif

#ifndef RAD_KERNEL_MAX_SPI_DEVICES
#define RAD_KERNEL_MAX_SPI_DEVICES 32u
#endif

#ifndef RAD_KERNEL_MAX_BUS_DRIVERS
#define RAD_KERNEL_MAX_BUS_DRIVERS 16u
#endif

#ifndef RAD_KERNEL_MAX_DMA_CONTROLLERS
#define RAD_KERNEL_MAX_DMA_CONTROLLERS 4u
#endif

#ifndef RAD_KERNEL_MAX_DMA_CHANNELS
#define RAD_KERNEL_MAX_DMA_CHANNELS 16u
#endif

#ifndef RAD_KERNEL_MAX_FRAMEBUFFERS
#define RAD_KERNEL_MAX_FRAMEBUFFERS 4u
#endif

#ifndef RAD_KERNEL_MAX_IRQ_DOMAINS
#define RAD_KERNEL_MAX_IRQ_DOMAINS 16u
#endif
...
#ifndef RAD_KERNEL_SPI_AUTO_DMA_THRESHOLD
#define RAD_KERNEL_SPI_AUTO_DMA_THRESHOLD 64u
#endif
...
struct OverlayState {
    TreeNode nodes[RAD_KERNEL_MAX_TREE_NODES];
    TreeProperty props[RAD_KERNEL_MAX_TREE_PROPS];
    OverlayRecord overlays[RAD_KERNEL_MAX_OVERLAYS];
    I2cController i2c[RAD_KERNEL_MAX_I2C_CONTROLLERS];
    I2cDriver i2c_drivers[RAD_KERNEL_MAX_BUS_DRIVERS];
    rad_i2c_device i2c_devices[RAD_KERNEL_MAX_I2C_DEVICES];
    SpiController spi[RAD_KERNEL_MAX_SPI_CONTROLLERS];
    SpiDriver spi_drivers[RAD_KERNEL_MAX_BUS_DRIVERS];
    rad_spi_device spi_devices[RAD_KERNEL_MAX_SPI_DEVICES];
    DmaController dma[RAD_KERNEL_MAX_DMA_CONTROLLERS];
    rad_dma_channel_handle dma_channels[RAD_KERNEL_MAX_DMA_CHANNELS];
    FramebufferRecord framebuffers[RAD_KERNEL_MAX_FRAMEBUFFERS];
    rad_irq_domain_handle irq_domains[RAD_KERNEL_MAX_IRQ_DOMAINS];
    ServiceRecord services[RAD_KERNEL_MAX_SERVICES];
};
...
TreeNode *find_node_mut(const char *path) {
    ...
    for (size_t i = 0; i < RAD_KERNEL_MAX_TREE_NODES; ++i) {
...
TreeProperty *find_property_mut(TreeNode *node, const char *name) {
    ...
    for (uint32_t i = 0; i < node->property_count && i < 16u; ++i) {
        ...
        if (index < RAD_KERNEL_MAX_TREE_PROPS && g_overlay.props[index].used && strcmp(g_overlay.props[index].name, name) == 0) {
...
TreeProperty *allocate_property(TreeNode *node, const char *name) {
    ...
    for (size_t i = 0; i < RAD_KERNEL_MAX_TREE_PROPS; ++i) {
...
OverlayRecord *allocate_overlay_record() {
    for (size_t i = 0; i < RAD_KERNEL_MAX_OVERLAYS; ++i) {
...
I2cController *find_i2c(uint32_t bus_id) {
    for (size_t i = 0; i < RAD_KERNEL_MAX_I2C_CONTROLLERS; ++i) {
...
SpiController *find_spi(uint32_t bus_id) {
    for (size_t i = 0; i < RAD_KERNEL_MAX_SPI_CONTROLLERS; ++i) {
...
rad_irq_domain_handle *find_irq_domain_by_path(const char *tree_path) {
    ...
    for (size_t i = 0; i < RAD_KERNEL_MAX_IRQ_DOMAINS; ++i) {
...
I2cDriver *find_i2c_driver_by_name(const char *name) {
    ...
    for (size_t i = 0; i < RAD_KERNEL_MAX_BUS_DRIVERS; ++i) {
...
rad_i2c_device *find_i2c_device(uint32_t bus_id, uint8_t address) {
    for (size_t i = 0; i < RAD_KERNEL_MAX_I2C_DEVICES; ++i) {
...
rad_spi_device *find_spi_device(uint32_t bus_id, uint8_t cs) {
    for (size_t i = 0; i < RAD_KERNEL_MAX_SPI_DEVICES; ++i) {
...
int dma_backend_available(void) {
    for (size_t i = 0; i < RAD_KERNEL_MAX_DMA_CONTROLLERS; ++i) {
...
FramebufferRecord *find_framebuffer(const char *name) {
    if (!name) return nullptr;
    for (size_t i = 0; i < RAD_KERNEL_MAX_FRAMEBUFFERS; ++i) {
...
FramebufferRecord *framebuffer_from_handle(rad_framebuffer_t framebuffer) {
    if (!framebuffer || framebuffer->index >= RAD_KERNEL_MAX_FRAMEBUFFERS) return nullptr;
...
ServiceRecord *find_service_by_name(const char *name) {
    if (!name) return nullptr;
    for (size_t i = 0; i < RAD_KERNEL_MAX_SERVICES; ++i) {
...
rad_status_t bind_spi_tree(void) {
    for (size_t i = 0; i < RAD_KERNEL_MAX_TREE_NODES; ++i) {
        ...
        if (!device) {
            for (size_t d = 0; d < RAD_KERNEL_MAX_SPI_DEVICES; ++d) {
...
rad_status_t rad_framebuffer_register_ex(const rad_framebuffer_config_t *config, const rad_framebuffer_ops_t *ops) {
    ...
    for (size_t i = 0; i < RAD_KERNEL_MAX_FRAMEBUFFERS; ++i) {
        ...
        if (fb.primary) {
            for (size_t other = 0; other < RAD_KERNEL_MAX_FRAMEBUFFERS; ++other) {
...
rad_status_t rad_framebuffer_set_primary(const char *name) {
    FramebufferRecord *fb = find_framebuffer(name);
    if (!fb) return RAD_STATUS_NOT_FOUND;
    for (size_t i = 0; i < RAD_KERNEL_MAX_FRAMEBUFFERS; ++i) g_overlay.framebuffers[i].primary = 0;
    fb->primary = 1;
    return RAD_STATUS_OK;
}
...
rad_status_t rad_i2c_controller_unregister(uint32_t bus_id) {
    ...
    for (size_t i = 0; i < RAD_KERNEL_MAX_I2C_DEVICES; ++i) {
        ...
        if (device.bound && device.driver_index < RAD_KERNEL_MAX_BUS_DRIVERS) {
...
rad_status_t rad_i2c_bind_tree(void) {
    for (size_t i = 0; i < RAD_KERNEL_MAX_TREE_NODES; ++i) {
        ...
        if (!device) {
            for (size_t d = 0; d < RAD_KERNEL_MAX_I2C_DEVICES; ++d) {
...
rad_status_t rad_spi_device_transfer(rad_spi_device_t *device, const rad_spi_transfer_t *transfer) {
    ...
    const int wants_dma = routed.transfer_mode == RAD_SPI_TRANSFER_MODE_DMA
        || (routed.transfer_mode == RAD_SPI_TRANSFER_MODE_AUTO
            ...
            && routed.size >= RAD_KERNEL_SPI_AUTO_DMA_THRESHOLD
...
rad_status_t rad_dma_controller_unregister(const char *name) {
    ...
    for (size_t i = 0; i < RAD_KERNEL_MAX_DMA_CONTROLLERS; ++i) {
        ...
        for (size_t ch = 0; ch < RAD_KERNEL_MAX_DMA_CHANNELS; ++ch) {
...
rad_status_t rad_dma_channel_request(rad_dma_request_id_t request_id, rad_dma_channel_t *channel) {
    ...
    for (size_t controller_index = 0; controller_index < RAD_KERNEL_MAX_DMA_CONTROLLERS; ++controller_index) {
        DmaController& controller = g_overlay.dma[controller_index];
        if (!controller.used || controller.channel_count == 0) continue;
        for (uint32_t channel_index = 0; channel_index < controller.channel_count && channel_index < RAD_KERNEL_MAX_DMA_CHANNELS; ++channel_index) {
            int in_use = 0;
            for (size_t scan = 0; scan < RAD_KERNEL_MAX_DMA_CHANNELS; ++scan) {
            ...
            if (in_use) continue;
            for (size_t slot = 0; slot < RAD_KERNEL_MAX_DMA_CHANNELS; ++slot) {
...
void rad_dma_channel_release(rad_dma_channel_t channel) {
    if (!channel || !channel->used || channel->controller_index >= RAD_KERNEL_MAX_DMA_CONTROLLERS) return;
...
rad_status_t rad_dma_submit(rad_dma_channel_t channel, const rad_dma_transfer_t *transfer) {
    if (!channel || !channel->used || !transfer || channel->controller_index >= RAD_KERNEL_MAX_DMA_CONTROLLERS) return RAD_STATUS_INVALID_ARGUMENT;
    DmaController& controller = g_overlay.dma[channel->controller_index];
...
rad_status_t rad_dma_wait(rad_dma_channel_t channel, uint32_t timeout_ms) {
    if (!channel || !channel->used || channel->controller_index >= RAD_KERNEL_MAX_DMA_CONTROLLERS) return RAD_STATUS_INVALID_ARGUMENT;
    DmaController& controller = g_overlay.dma[channel->controller_index];
    if (!controller.used) return RAD_STATUS_NOT_SUPPORTED;
    rad_status_t status = RAD_STATUS_OK;
