...
#ifndef RADIX_KERNEL_NEWLIB_MAX_FDS
#define RADIX_KERNEL_NEWLIB_MAX_FDS 16
#endif

#ifndef RADIX_KERNEL_NEWLIB_HEAP_BYTES
#define RADIX_KERNEL_NEWLIB_HEAP_BYTES (256u * 1024u)
#endif
...
FdEntry g_fds[RADIX_KERNEL_NEWLIB_MAX_FDS]{};
alignas(16) unsigned char g_heap[RADIX_KERNEL_NEWLIB_HEAP_BYTES]{};
...
int alloc_fd(rad_file_t file, int flags) {
    lock_fds();
    for (int fd = 3; fd < RADIX_KERNEL_NEWLIB_MAX_FDS; ++fd) {
...
FdEntry *lookup_fd(int fd) {
    if (fd < 3 || fd >= RADIX_KERNEL_NEWLIB_MAX_FDS || !g_fds[fd].used) return nullptr;
    return &g_fds[fd];
}
