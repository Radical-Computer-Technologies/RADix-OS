...
static int sockaddr_to_rad(const struct sockaddr *src, socklen_t len, rad_sockaddr_in_t *dst) {
...
static void sockaddr_from_rad(const rad_sockaddr_in_t *src, struct sockaddr *dst, socklen_t *len) {
...
ssize_t read(int fd, void *buf, size_t count) {
    return ok_or_errno(rad_syscall6(RAD_SYS_READ, fd, (long)buf, count, 0, 0, 0));
}

ssize_t write(int fd, const void *buf, size_t count) {
    return ok_or_errno(rad_syscall6(RAD_SYS_WRITE, fd, (long)buf, count, 0, 0, 0));
}

int open(const char *pathname, int flags, ...) {
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_OPEN, (long)pathname, flags, 0, 0, 0, 0));
}
...
int close(int fd) {
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_CLOSE, fd, 0, 0, 0, 0, 0));
}

off_t lseek(int fd, off_t offset, int whence) {
    return ok_or_errno(rad_syscall6(RAD_SYS_LSEEK, fd, offset, whence, 0, 0, 0));
}

static void copy_stat(struct stat *dst, const rad_stat_t *src) {
    dst->st_dev = 0;
    dst->st_ino = 0;
...
int stat(const char *pathname, struct stat *statbuf) {
    rad_stat_t st;
    long r = rad_syscall6(RAD_SYS_STAT, (long)pathname, (long)&st, 0, 0, 0, 0);
...
int fstat(int fd, struct stat *statbuf) {
    rad_stat_t st;
    long r = rad_syscall6(RAD_SYS_FSTAT, fd, (long)&st, 0, 0, 0, 0);
    if (r < 0) return map_error(r);
    copy_stat(statbuf, &st);
    return 0;
}
...
int ioctl(int fd, unsigned long request, ...) {
    ...
    va_end(ap);
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_IOCTL, fd, request, (long)arg, 0, 0, 0));
}
...
int fcntl(int fd, int cmd, ...) {
    ...
    va_end(ap);
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_FCNTL, fd, cmd, arg, 0, 0, 0));
}

int poll(struct pollfd *fds, nfds_t nfds, int timeout) {
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_POLL, (long)fds, nfds, timeout, 0, 0, 0));
}

int socket(int domain, int type, int protocol) {
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_SOCKET, domain, type, protocol, 0, 0, 0));
}

int bind(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
    rad_sockaddr_in_t rad_addr;
    if (sockaddr_to_rad(addr, addrlen, &rad_addr) < 0) return -1;
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_BIND, sockfd, (long)&rad_addr, sizeof(rad_addr), 0, 0, 0));
}

int connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen) {
    rad_sockaddr_in_t rad_addr;
    if (sockaddr_to_rad(addr, addrlen, &rad_addr) < 0) return -1;
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_CONNECT, sockfd, (long)&rad_addr, sizeof(rad_addr), 0, 0, 0));
}

int listen(int sockfd, int backlog) {
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_LISTEN, sockfd, backlog, 0, 0, 0, 0));
}

int accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen) {
    rad_sockaddr_in_t rad_addr;
    size_t rad_len = sizeof(rad_addr);
    long r = rad_syscall6(RAD_SYS_ACCEPT, sockfd, (long)(addr ? &rad_addr : 0), (long)(addrlen ? &rad_len : 0), 0, 0, 0);
    if (r < 0) return map_error(r);
    if (addr && addrlen) sockaddr_from_rad(&rad_addr, addr, addrlen);
    return (int)r;
}

int shutdown(int sockfd, int how) {
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_SHUTDOWN, sockfd, how, 0, 0, 0, 0));
}

ssize_t sendto(int sockfd, const void *buf, size_t len, int flags, const struct sockaddr *dest_addr, socklen_t addrlen) {
    rad_sockaddr_in_t rad_addr;
    if (sockaddr_to_rad(dest_addr, addrlen, &rad_addr) < 0) return -1;
    return ok_or_errno(rad_syscall6(RAD_SYS_SENDTO, sockfd, (long)buf, len, flags, (long)&rad_addr, sizeof(rad_addr)));
}

ssize_t recvfrom(int sockfd, void *buf, size_t len, int flags, struct sockaddr *src_addr, socklen_t *addrlen) {
    rad_sockaddr_in_t rad_addr;
    size_t rad_len = sizeof(rad_addr);
    long r = rad_syscall6(RAD_SYS_RECVFROM, sockfd, (long)buf, len, flags, (long)(src_addr ? &rad_addr : 0), (long)(addrlen ? &rad_len : 0));
    if (r < 0) return map_error(r);
    if (src_addr && addrlen) sockaddr_from_rad(&rad_addr, src_addr, addrlen);
    return r;
}

ssize_t send(int sockfd, const void *buf, size_t len, int flags) {
    return ok_or_errno(rad_syscall6(RAD_SYS_SENDTO, sockfd, (long)buf, len, flags, 0, 0));
}

ssize_t recv(int sockfd, void *buf, size_t len, int flags) {
    return ok_or_errno(rad_syscall6(RAD_SYS_RECVFROM, sockfd, (long)buf, len, flags, 0, 0));
}
...
static uint64_t resolver_now_millis(void) {
    struct timeval tv;
    long status = rad_syscall6(RAD_SYS_GETTIMEOFDAY, (long)&tv, 0, 0, 0, 0, 0);
    if (status < 0) return 0;
    return (uint64_t)tv.tv_sec * 1000u + (uint64_t)(tv.tv_usec / 1000u);
}

static void resolver_sleep_ms(uint32_t ms) {
    rad_syscall6(RAD_SYS_NANOSLEEP, (long)ms * 1000000L, 0, 0, 0, 0, 0);
}
...
int tcgetattr(int fd, struct termios *termios_p) {
    ...
    long r = rad_syscall6(RAD_SYS_IOCTL, fd, RAD_IOR('Y', 5u, struct termios), (long)termios_p, 0, 0, 0);
    if (r < 0) return map_error(r);
    return 0;
}
...
int tcsetattr(int fd, int optional_actions, const struct termios *termios_p) {
    ...
    int status = (int)ok_or_errno(rad_syscall6(RAD_SYS_IOCTL, fd, RAD_IOW('Y', 6u, struct termios), (long)termios_p, 0, 0, 0));
    if (status < 0) return status;
...
int tcflush(int fd, int queue_selector) {
    ...
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_IOCTL, fd, RAD_IOW('Y', 7u, uint32_t), (long)&queues, 0, 0, 0));
}
...
int chdir(const char *path) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_CHDIR, (long)path, 0, 0, 0, 0, 0)); }
int fchdir(int fd) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_FCHDIR, fd, 0, 0, 0, 0, 0)); }
char *getcwd(char *buf, size_t size) { return rad_syscall6(RAD_SYS_GETCWD, (long)buf, size, 0, 0, 0, 0) < 0 ? 0 : buf; }
...
int isatty(int fd) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_ISATTY, fd, 0, 0, 0, 0, 0)); }
int pipe(int pipefd[2]) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_PIPE, (long)pipefd, 0, 0, 0, 0, 0)); }
int pipe2(int pipefd[2], int flags) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_PIPE2, (long)pipefd, flags, 0, 0, 0, 0)); }
int dup(int oldfd) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_DUP, oldfd, 0, 0, 0, 0, 0)); }
int dup2(int oldfd, int newfd) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_DUP2, oldfd, newfd, 0, 0, 0, 0)); }
int getdtablesize(void) { return 256; }
pid_t fork(void) { return (pid_t)ok_or_errno(rad_syscall6(RAD_SYS_FORK, 0, 0, 0, 0, 0, 0)); }
int execve(const char *pathname, char *const argv[], char *const envp[]) { (void)envp; return (int)ok_or_errno(rad_syscall6(RAD_SYS_EXECVE, (long)pathname, (long)argv, 0, 0, 0, 0)); }
...
pid_t waitpid(pid_t pid, int *wstatus, int options) { return (pid_t)ok_or_errno(rad_syscall6(RAD_SYS_WAITPID, pid, (long)wstatus, options, 0, 0, 0)); }
pid_t wait(int *wstatus) { return waitpid(-1, wstatus, 0); }
pid_t getpid(void) { return (pid_t)rad_syscall6(RAD_SYS_GETPID, 0, 0, 0, 0, 0, 0); }
pid_t getppid(void) { return (pid_t)rad_syscall6(RAD_SYS_GETPPID, 0, 0, 0, 0, 0, 0); }
pid_t getpgrp(void) { return (pid_t)rad_syscall6(RAD_SYS_GETPGID, 0, 0, 0, 0, 0, 0); }
pid_t getpgid(pid_t pid) { return (pid_t)ok_or_errno(rad_syscall6(RAD_SYS_GETPGID, pid, 0, 0, 0, 0, 0)); }
int setpgid(pid_t pid, pid_t pgid) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_SETPGID, pid, pgid, 0, 0, 0, 0)); }
pid_t getsid(pid_t pid) { return (pid_t)ok_or_errno(rad_syscall6(RAD_SYS_GETSID, pid, 0, 0, 0, 0, 0)); }
pid_t setsid(void) { return (pid_t)ok_or_errno(rad_syscall6(RAD_SYS_SETSID, 0, 0, 0, 0, 0, 0)); }
pid_t tcgetpgrp(int fd) { return (pid_t)ok_or_errno(rad_syscall6(RAD_SYS_TCGETPGRP, fd, 0, 0, 0, 0, 0)); }
int tcsetpgrp(int fd, pid_t pgrp) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_TCSETPGRP, fd, pgrp, 0, 0, 0, 0)); }
uid_t getuid(void) { return (uid_t)rad_syscall6(RAD_SYS_GETUID, 0, 0, 0, 0, 0, 0); }
uid_t geteuid(void) { return (uid_t)rad_syscall6(RAD_SYS_GETEUID, 0, 0, 0, 0, 0, 0); }
gid_t getgid(void) { return (gid_t)rad_syscall6(RAD_SYS_GETGID, 0, 0, 0, 0, 0, 0); }
gid_t getegid(void) { return (gid_t)rad_syscall6(RAD_SYS_GETEGID, 0, 0, 0, 0, 0, 0); }
...
int unlink(const char *pathname) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_REMOVE, (long)pathname, 0, 0, 0, 0, 0)); }
int rmdir(const char *pathname) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_RMDIR, (long)pathname, 0, 0, 0, 0, 0)); }
int mkdir(const char *pathname, mode_t mode) {
    int r = (int)ok_or_errno(rad_syscall6(RAD_SYS_MKDIR, (long)pathname, 0, 0, 0, 0, 0));
    if (r == 0 && mode) (void)chmod(pathname, mode & ~g_process_umask);
    return r;
}
int chmod(const char *pathname, mode_t mode) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_CHMOD, (long)pathname, mode, 0, 0, 0, 0)); }
mode_t umask(mode_t mask) {
    mode_t previous = g_process_umask;
    g_process_umask = mask & 0777;
    return previous;
}
int symlink(const char *target, const char *linkpath) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_SYMLINK, (long)target, (long)linkpath, 0, 0, 0, 0)); }
int link(const char *oldpath, const char *newpath) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_LINK, (long)oldpath, (long)newpath, 0, 0, 0, 0)); }
ssize_t readlink(const char *pathname, char *buf, size_t bufsiz) { return ok_or_errno(rad_syscall6(RAD_SYS_READLINK, (long)pathname, (long)buf, bufsiz, 0, 0, 0)); }
int utime(const char *filename, const struct utimbuf *times) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_UTIME, (long)filename, (long)times, 0, 0, 0, 0)); }
int fsync(int fd) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_FSYNC, fd, 0, 0, 0, 0, 0)); }
int nanosleep(const struct timespec *req, struct timespec *rem) { (void)rem; if (!req || req->tv_sec < 0 || req->tv_nsec < 0 || req->tv_nsec >= 1000000000L) { errno = EINVAL; return -1; } return (int)ok_or_errno(rad_syscall6(RAD_SYS_NANOSLEEP, req->tv_sec * 1000000000L + req->tv_nsec, 0, 0, 0, 0, 0)); }
unsigned int sleep(unsigned int seconds) { struct timespec ts = {(long)seconds, 0}; return nanosleep(&ts, 0) == 0 ? 0 : seconds; }
int gettimeofday(struct timeval *tv, void *tz) { (void)tz; return (int)ok_or_errno(rad_syscall6(RAD_SYS_GETTIMEOFDAY, (long)tv, 0, 0, 0, 0, 0)); }
int settimeofday(const struct timeval *tv, const void *tz) { (void)tz; if (!tv) { errno = EINVAL; return -1; } return (int)ok_or_errno(rad_syscall6(RAD_SYS_SETTIMEOFDAY, (long)tv, 0, 0, 0, 0, 0)); }
...
int kill(int pid, int sig) {
    ...
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_KILL, pid, sig, 0, 0, 0, 0));
}
int raise(int sig) {
    if (sig <= 0 || sig >= NSIG) { errno = EINVAL; return -1; }
    init_signal_actions();
...
struct dirent *readdir(DIR *dirp) {
    ...
    rad_dirent_t ent;
    long n = rad_syscall6(RAD_SYS_GETDENTS, dirp->fd, (long)&ent, 1, 0, 0, 0);
    if (n <= 0) return 0;
...
/* RAD_LIBC_NO_FLOAT: strip all floating-point entry points so freestanding
 * targets can build the libc with -mgeneral-regs-only (no FP/SIMD registers). */
#ifndef RAD_LIBC_NO_FLOAT
...
typedef struct rad_tnode {
    const void *key;
    struct rad_tnode *left;
    struct rad_tnode *right;
} rad_tnode_t;

void *tfind(const void *key, void *const *rootp, int (*compar)(const void *, const void *)) {
    if (!rootp || !compar) return 0;
    rad_tnode_t *node = (rad_tnode_t*)*rootp;
    while (node) {
...
void *tsearch(const void *key, void **rootp, int (*compar)(const void *, const void *)) {
    if (!rootp || !compar) return 0;
    rad_tnode_t **slot = (rad_tnode_t**)rootp;
    while (*slot) {
        int cmp = compar(key, (*slot)->key);
        if (cmp == 0) return &(*slot)->key;
        slot = cmp < 0 ? &(*slot)->left : &(*slot)->right;
    }
    rad_tnode_t *node = (rad_tnode_t*)calloc(1, sizeof(*node));
    if (!node) return 0;
    node->key = key;
    *slot = node;
    return &node->key;
...
static rad_tnode_t *tdelete_min(rad_tnode_t **rootp) {
    rad_tnode_t **slot = rootp;
    while ((*slot)->left) slot = &(*slot)->left;
    rad_tnode_t *node = *slot;
    *slot = node->right;
    return node;
}
...
void *tdelete(const void *key, void **rootp, int (*compar)(const void *, const void *)) {
    ...
    while (*slot) {
        ...
        if (cmp < 0) {
            ...
            rad_tnode_t *victim = *slot;
            if (!victim->left) {
                *slot = victim->right;
            } else if (!victim->right) {
                *slot = victim->left;
            } else {
                rad_tnode_t *replacement = tdelete_min(&victim->right);
...
static void twalk_node(const rad_tnode_t *node, void (*action)(const void *, VISIT, int), int depth) {
...
void twalk(const void *root, void (*action)(const void *, VISIT, int)) {
    twalk_node((const rad_tnode_t*)root, action, 0);
}

void tdestroy(void *root, void (*free_node)(void *nodep)) {
    rad_tnode_t *node = (rad_tnode_t*)root;
    if (!node) return;
...
void _exit(int status) { rad_syscall6(RAD_SYS_EXIT, status, 0, 0, 0, 0, 0); for (;;) {} }
...
int rename(const char *oldpath, const char *newpath) { return (int)ok_or_errno(rad_syscall6(RAD_SYS_RENAME, (long)oldpath, (long)newpath, 0, 0, 0, 0)); }
void perror(const char *s) { if (s && *s) { dprintf(2, "%s: ", s); } dprintf(2, "%s\n", strerror(errno)); }

int truncate(const char *path, off_t length) {
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_TRUNCATE, (long)path, length, 0, 0, 0, 0));
}

int ftruncate(int fd, off_t length) {
    return (int)ok_or_errno(rad_syscall6(RAD_SYS_FTRUNCATE, fd, length, 0, 0, 0, 0));
}
