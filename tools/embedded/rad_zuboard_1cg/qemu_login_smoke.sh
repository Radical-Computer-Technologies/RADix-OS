...
# login (root/rad) into rash and exercises ls/cat/ps. Complements
# qemu_marker_smoke.sh (non-interactive marker gate).
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
LOG_FILE="${RAD_ZUBOARD_LOGIN_LOG:-${TMPDIR:-/tmp}/rad-zuboard-login-smoke.log}"
LOGIN_USER="${RAD_ZUBOARD_LOGIN_USER:-root}"
LOGIN_PASSWORD="${RAD_ZUBOARD_LOGIN_PASSWORD:-rad}"
BOOT_TIMEOUT="${RAD_ZUBOARD_LOGIN_BOOT_TIMEOUT:-75}"
STEP_TIMEOUT="${RAD_ZUBOARD_LOGIN_STEP_TIMEOUT:-15}"

export RAD_QEMU_TIMEOUT="${RAD_QEMU_TIMEOUT:-120}"
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/rad-login-smoke.XXXXXX")"
...
send "$LOGIN_PASSWORD"
await "RAD_LOGIN_OK" "$STEP_TIMEOUT"    || fail "login accepted"
send "ls /"
await "bin" "$STEP_TIMEOUT"               || fail "ls output"
send "cat /etc/hostname"
await "rad" "$STEP_TIMEOUT"             || fail "cat output"
send "ps"
