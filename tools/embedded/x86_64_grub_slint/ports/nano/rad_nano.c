...
#ifndef RAD_NANO_FULL
#define RAD_NANO_FULL 1
#endif

#if RAD_NANO_FULL
#define RAD_NANO_VARIANT "nano-full"
#else
#define RAD_NANO_VARIANT "nano-tiny"
#endif
...
static void draw_screen(const char *path, const char *buffer, size_t size, int saved) {
    puts_fd(1, "\x1b[2J\x1b[H\x1b[95mRAD ");
    puts_fd(1, RAD_NANO_VARIANT);
    puts_fd(1, "\x1b[0m ");
...
static int version(void) {
    puts_fd(1, RAD_NANO_VARIANT);
    puts_fd(1, " (GNU nano 9.0 RADPx-OS port");
#if RAD_NANO_FULL
...
#endif
    puts_fd(1, ")\n");
    marker(RAD_NANO_FULL ? "RAD_NANO_FULL_VERSION_OK" : "RAD_NANO_TINY_VERSION_OK");
    return 0;
}

static int smoke(const char *path) {
    const char *text = RAD_NANO_FULL ? "RAD_NANO_FULL_EDIT_OK\n" : "RAD_NANO_TINY_EDIT_OK\n";
    int status = save_file(path, text, s_len(text));
    if (status == 0) {
        marker(RAD_NANO_FULL ? "RAD_NANO_FULL_SAVE_OK" : "RAD_NANO_TINY_SAVE_OK");
        marker(RAD_NANO_FULL ? "RAD_NANO_FULL_EXIT_OK" : "RAD_NANO_TINY_EXIT_OK");
    }
    return status;
}

int rad_nano_main(long argc, char **argv, char **envp) {
    (void)envp;
    if (argc > 1 && s_eq(argv[1], "--version")) return version();
    if (argc > 2 && s_eq(argv[1], "--rad-smoke")) return smoke(argv[2]);
    ...
    marker(RAD_NANO_FULL ? "RAD_NANO_FULL_OPEN_OK" : "RAD_NANO_TINY_OPEN_OK");
